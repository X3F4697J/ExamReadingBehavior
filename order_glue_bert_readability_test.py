...
#for data_type, lists in zip([type1, type2, type3, type4, type5, type6],[type1_list,type2_list,type3_list,type4_list,type5_list,type6_list]):
for data_type, lists in zip([type7],[type7_list]):
    ...
    for seed_idx, seed in enumerate([7800, 8321, 7084, 8147, 15000]):
        ...
        for hardness_idx, train_data in enumerate(data_type):
            ...
            eval_dataset = ReadabilityGlueDataset(whole_evaluation_data)
                    ...
                    if (batch_idx % evaluation_dict[task] == 0) & (batch_idx != 0):
                        ...
                        with torch.no_grad():
                            for eval_batch in tqdm(eval_loader):
                                ...
                                temp_loss.append(output['loss'].item())
                        ...
                        loss_list.append(np.array(temp_loss).mean().item())
                        model.train()
...
if assessement == 'uid':
    ...
    pass
f = open('bert_accu_loss_list_'+task+'_'+'normal'+'.pkl','wb')
